...
namespace H.RedisTools
{
    ...
    public class dicEntity
    {
        ...
        public decimal? un_total { get; set; }
    ...
    public class AuthyEntity
    {
        public string name { get; set; }
        public string value { get; set; }
        public bool @checked { get; set; }
        public bool disabled { get; set; }
        public List<AuthyEntity> list { get; set; }
    }
    public class MeneEntity
    {
        public string menuid { get; set; }
        public string menuname { get; set; }
        public string url { get; set; }
        public string icon { get; set; }
        public List<MeneEntity> child { get; set; }
    }
    /// <summary>
    /// 分页参数
    /// </summary>
    ...
    public class MessageInfo
    {
        ...
        public string msg { get; set; }
        public int type { get; set; }//0系统管理员，1简单用户
        public int showType { get; set; }//0通知信息，1订单信息
    ...
    public class RedisHelper
    {
        ...
        /// <param name="value">保存的值</param>
        /// <param name="expiry">过期时间</param>
        /// <param name="DType">过期时间</param>
        /// <returns></returns>
        public bool Set(string key, string value, int cashTime, DType DType = DType.Seconds)
        {
            key = AddSysCustomKey(key);
            TimeSpan timeSpan = TimeSpan.FromSeconds(cashTime);
            if (DType == DType.Minutes)
                timeSpan = TimeSpan.FromMinutes(cashTime);
            else if (DType == DType.Day)
                timeSpan = TimeSpan.FromDays(cashTime);
            else if (DType == DType.Hours)
                timeSpan = TimeSpan.FromHours(cashTime);
            else if (DType == DType.Milliseconds)
                timeSpan = TimeSpan.FromMilliseconds(cashTime);
            return Do(db => db.StringSet(key, value, timeSpan));
        }
        /// <summary>
        /// 保存一个对象
        /// </summary>
        ...
        /// <param name="expiry"></param>
        /// <returns></returns>
        public bool Set<T>(string key, T obj, int cashTime = 30, DType DType = DType.Day)
        {
            key = AddSysCustomKey(key);
            TimeSpan timeSpan = TimeSpan.FromSeconds(cashTime);
            if (DType == DType.Minutes)
            ...
            string json = ConvertJson(obj);
            return Do(db => db.StringSet(key, json, timeSpan));
        }
        ...
        {
            var list = HashKeys<string>(key).ToList();
            list.ForEach(t => HashDelete(key, t.ToString()));
        }
        #endregion
    }
}
